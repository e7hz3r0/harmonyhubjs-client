 ...
 *
 * @param xmppClient
 * @param isLink Optional. Boolean indicating if this is a Harmony Link device.
 * @constructor
 */
function HarmonyClient (xmppClient, isLink) {
...
function startActivity (activityId) {
  ...
  var body = 'activityId=' + activityId + (this._isHarmonyLink ? '' : ':timestamp=' + timestamp)

  var startString = (this._isHarmonyLink ? 'startActivity' : 'startactivity')
  return this.request(startString, body, 'encoded', function (stanza) {
...
function buildCommandIqStanza (command, body) {
  ...
  return xmppUtil.buildIqStanza(
    (this._isHarmonyLink ? command : 'get')
...
function request (command, body, expectedResponseType, canHandleStanzaPredicate) {
  ...
  var iq = this.buildCommandIqStanza(command, body)
  var id = iq.attr('id')
...
HarmonyClient.prototype.buildCommandIqStanza = buildCommandIqStanza
