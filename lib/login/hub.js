 ...
 *
 * Parameters:
 *     (String) authToken - Unique token received from myharmony.com via the auth module
 ...
 */
function getIdentity (authToken, hubhost, hubport) {
  ...
  xmppClient.on('online', function () {
    ...
    var body = 'token=' + authToken + ':name=harmonyjs#iOS6.0.1#iPhone';
 ...
 *                   Harmony hub.
 */
function loginToHub (username, password, hubhost, hubport) {
  ...
  return auth(username, password)
    .then(function (authToken) {
      return getIdentity(authToken, hubhost, hubport)
